...
let
  ...
  # buildEnv composes an environment with symlinks to the appropriate package directories.
  packageEnv = buildEnv {
    name = "elisp-packages";
    paths = elispInputs;
    pathsToLink = [
  ...
  lispList = strings: wrap "'(" ")" (lib.concatMapStringsSep " " (wrap "\"" "\"") strings);

  nativeLoadPath = "${packageEnv}/share/emacs/native-lisp/";

  infoPath = "${packageEnv}/share/info";

  elispManifest = writeText "elisp-digest.json" (
    builtins.toJSON {
      inherit configurationRevision;
      emacsPath = emacs.outPath;
      inherit nativeLoadPath infoPath;
      elispPackages = lib.genAttrs packageNames (name: "${elispPackages.${name}}/share/emacs/site-lisp/");
...
runCommandLocal "emacs"
  {
    ...
    # Where ahead-of-time native-compiled packages livesc
    nativeLoadPath = "${nativeLoadPath}:${emacs}/share/emacs/native-lisp/:";

    # Tell emacs where the compiled packages are
    subdirs = ''
    ...
    elispManifestPath = if exportManifest then elispManifest.outPath else null;
  }
  ''
    mkdir -p $out/bin
