...
impl<'a> Query<'a> {
    ...
    pub fn parse(from: &'a str) -> Query<'a> {
        ...
        Isbn::parse(trimmed)
            .map(|_| Query::Isbn(Cow::Borrowed(trimmed)))
            .or_else(|| Doi::parse(trimmed).map(|doi| Query::Doi(doi.into_cow())))
            .unwrap_or(Query::FullText(from))
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::query::doi::Doi;
    use crate::query::isbn::{Isbn, IsbnInput};
    ...
    fn strip_doi_trailing_decor<'a>(s: &'a str) -> Cow<'a, str> {
        Doi::strip_trailing_borrowed(s).inner()
    }
    ...
    fn strip_outer_wrappers_handles_too_short() {
        assert_eq!(IsbnInput::strip_outer_wrappers(""), None);
        assert_eq!(IsbnInput::strip_outer_wrappers("("), None);
        assert_eq!(IsbnInput::strip_outer_wrappers(")"), None);
    }
